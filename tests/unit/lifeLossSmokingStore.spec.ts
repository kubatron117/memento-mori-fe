...
describe('LifeLossSmokingStore', () => {
  ...
  it('correctly calls updateMainStore and updates questionnaireStore', () => {
    ...
    expect(questionnaireStoreMock.updateField).toHaveBeenCalledWith(
      'smokingDaysLost',
      store.daysLost
    );
    expect(questionnaireStoreMock.updateField).toHaveBeenCalledWith(
      'smokingAdditionalDaysLost',
      store.additionalDaysLost
    );
  });
});
